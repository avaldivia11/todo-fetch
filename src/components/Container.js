...
/* const Container = () => {
  ...
  const [url] = useState(
    "https://assets.breatheco.de/apis/fake/todos/user/andresv11"
  );
    ...
    />
      ...
      <button
       className="button pink"
        onClick={() => {
          console.log(label);
          setList([...list, { label: label, done: false }]);
        }}>
        agregar
      </button>
      <div className='todo-list'>
      {list.map((element, key) => {
        return <li key={key}>{element.label} 
        
        <button onClick={()=>{
          console.log(key)
          let newArray=[]
          list.filter(function(elemento, key) {
            if (elemento != key) {
          ...
          setList(newArray);
          /*cuando logres eliminar osea actualizar lista debes llamar funcion updateTask*/
